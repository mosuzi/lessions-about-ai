...
模型看到“它”时，需要判断“它”更可能指“书”还是“书包”。attention 做的事就是让“它”去看上下文里的其他词：

- “书”很相关。
- “书包”也相关，但可能没那么相关。
- “小明”不太相关。

最后模型会把这些位置的信息按相关性权重汇总，得到“它”这个位置更合适的上下文表示。

所以 attention 不是心理学意义上的玄学“注意力”。在 Transformer 里，它更像一个可计算的查询过程：每个 token 都可以直接读取上下文里其他 token 的信息。
